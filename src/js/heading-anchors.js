...
class HeadingAnchors extends HTMLElement {
	...
	static tagName = 'heading-anchors';

	static register(tagName = HeadingAnchors.tagName) {
		if ('customElements' in window && !customElements.get(tagName)) {
			customElements.define(tagName, HeadingAnchors);
		}
	}
	...
	static defaultSelector = 'h1,h2,h3,h4,h5,h6';
...
@supports (anchor-name: none) {
	...
	constructor() {
		...
		if (!this.supportsReplaceSheet) return;
	...
	positionAnchor(anchor) {
		...
		// quit early
		if (this.supportsAnchorPosition) return;
	...
	getAnchorElement(heading) {
		...
		anchor.innerHTML = `<span class="${
			HeadingAnchors.classes.srOnly
		}">${this.getAccessibleTextPrefix()}: ${
			heading.textContent
		}</span><span aria-hidden="true">${content}</span>`;
